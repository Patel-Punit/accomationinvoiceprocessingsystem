...
def file_to_response_json_accomation(uploaded_file, 
                            upload_url: str = 'http://15.207.159.131:3200/api/pdf-ocr-connector/upload', 
                            api_key: str = '4e9380bb211e1ae16f58f82a514ed8e905f31289356c8e79566511ce',
                            max_retries: int = 48,
                            wait_time: int = 5) -> Optional[Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]]:
    """
    Upload a PDF file to OCR API and process the response with retry mechanism.
    
    Args:
        uploaded_file: Streamlit uploaded file object
        upload_url (str): Upload API endpoint URL
        api_key (str): API authentication key
        max_retries (int): Maximum number of times to check job status
        wait_time (int): Seconds to wait between status checks
    
    Returns:
        Tuple of three DataFrames: (invoice_df, total_summary_df, line_items_df)
        Returns None if upload or processing fails
    """
    try:
        # Upload PDF file directly from Streamlit uploaded file
        ...
        job_id = upload_response.json().get('data', {}).get('jobId')
        
        if not job_id:
            st.error("No job ID found in response")
            return None, None

        # Wait and retry mechanism
        for attempt in range(max_retries):
            # st.info(f"Checking job status (Attempt {attempt + 1}/{max_retries})...")
            
            # Wait before checking status
            time.sleep(wait_time)
            
            # Fetch job details
            job_url = f'http://15.207.159.131:3200/api/pdf-ocr-connector/{job_id}'
            ...
            if 'Ocr Data fetched successfully' in str(response_data.get('message', '')):
                data = response_data.get('data')
                
                if not data:
                    st.error("No data found in job response")
                    return None, None

                # Create DataFrames
                # invoice_df = pd.DataFrame([data.get('Invoice Details', {})])
                # total_summary_df = pd.DataFrame([data.get('Total Summary', {})])
                # line_items_df = pd.DataFrame(data.get('Line Items', []))

                return data, 200
            
            # If still processing, continue to next retry
            elif response_data.get('message') == "Data is in processing":
                continue
        ...
        st.error("Max retries reached. Processing did not complete.")
        return None, None
